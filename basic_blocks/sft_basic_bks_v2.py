...
def inten_FE(inten_feat, shape_list=[[7,7,1,64],[7,7,64,32]],stride=[1,1,1,1],skip_Prelu_list=[False,False]):
...
def dep_FUp(dep_feat,prev_dep_feat,shape=[5,5,32,32],stride=[1,1,1,1],skip_Prelu=[True,False]):
    dep_feat=conv_Prelu_block(dep_feat, shape, stride, skip_Prelu[0])
    dep_feat=dep_feat+prev_dep_feat
    dep_feat_up=deconv_Prelu_block(dep_feat,shape,[1,2,2,1],skip_Prelu[1])
    return dep_feat_up

def sft_layer(inten_feat,dep_feat,shape_list=[[5,5,64,32],[5,5,32,32]],stride=[1,1,1,1],skip_Prelu_list=[False,True]):
    fusion_ten=tf.concat([inten_feat,dep_feat],3)
    scale_feat=fusion_ten
    trans_feat=fusion_ten
    for index in range(len(skip_Prelu_list)):
        scale_feat=conv_Prelu_block(scale_feat, shape_list[index], stride, skip_Prelu_list[index])
        trans_feat=conv_Prelu_block(trans_feat, shape_list[index], stride, skip_Prelu_list[index])
    ref_inten_feat=tf.multiply(scale_feat,inten_feat)
    ref_inten_feat=tf.add(ref_inten_feat,trans_feat)
    return ref_inten_feat

def feat_fusion(ref_inten_feat,dep_feat,shape=[5,5,64,32],stride=[1,1,1,1],skip_Prelu=False):
    fusion_ten=tf.concat([ref_inten_feat,dep_feat],3)
    fusion_ten=conv_Prelu_block(fusion_ten, shape, stride, skip_Prelu)
    return fusion_ten
    
def recon(coar_dep,final_feat, shape_list=[[5,5,32,32],[5,5,32,32],[5,5,32,1]], stride=[1,1,1,1], skip_Prelu_list=[False,False,True]):
    for ind in range(len(skip_Prelu_list)):
