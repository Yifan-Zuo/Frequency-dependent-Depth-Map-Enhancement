...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        sess.run(init_op)
        #saver_full.restore(sess, "/media/kenny/Data/trained_models/multi_dense_guide_resnet/noisy/l2loss/4x/full_model1/4x_ny_full_model.ckpt-8")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/yifan/Data/trained_models/sft_depup_net/noise_free/l1loss/4x/full_model/4x_nf_cgsft_model.ckpt",global_step=model_ind)
