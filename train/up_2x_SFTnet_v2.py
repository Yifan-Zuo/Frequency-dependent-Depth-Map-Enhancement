...
epo_range=20
train_h5F_addr="/media/yifan/Data/training_data/gdsr_train_data/2x_data/shuffle_version/2x_training_data.h5"
total_pat=220416
total_val_pat=24500
...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        #sess.run(init_op)
        saver_full.restore(sess, "/media/yifan/Data/trained_models/sft_depup_net/noise_free/l1loss/2x/full_model/2x_nf_cgsft_model.ckpt-119")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/yifan/Data/trained_models/sft_depup_net/noise_free/l1loss/2x/full_model2/2x_nf_cgsft_model.ckpt",global_step=model_ind)
